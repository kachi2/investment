...
<div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
        ...
        <div class="modal-body modal-body-md">
            <h4 class="title">{{ blank($userAccount) ? __('Add Crypto Account') : __('Update Crypto Account') }}</h4>
            @if(blank($userAccount))
            <p>{{ __('Add your Crypto address to withdraw your funds.') }}</p>
            @else 
            <p>{{ __('Update your existing Crypto account for future withdraw.') }}</p>
            @endif
                        ...
                        <div class="row gx-1">
                            <div class="col-8 col-md-9">
                                <div class="form-group">
                                    <label class="form-label" for="paypal-email">{{ __('Crypto address') }} <span class="text-danger">*</span></label>
                                    <div class="form-control-wrap">
                                        <input type="email" name="wdm-email" value="{{ data_get($userAccount, 'config.email') }}" class="form-control form-control-lg" id="paypal-email" placeholder="{{ __('Enter Crypto address') }}">                                    
                                    </div>
                                </div>
                            </div>
                ...
                <div class="row gy-3">
                    <div class="col-12">
                        ...
                        <ul class="align-center flex-wrap flex-sm-nowrap gx-4 gy-2">
                            ...
                            @if(!blank($userAccount))
                            <li>
                                <a href="javascript:void(0)" id="delete-wd-account" class="link link-btn link-danger" data-url="{{route('user.withdraw.account.wd-crypto.delete', ['id' => the_hash(data_get($userAccount, 'id', 0))])}}">{{ __('Delete') }}</a>
                            </li>
                            @endif
                        </ul>
                    </div>
