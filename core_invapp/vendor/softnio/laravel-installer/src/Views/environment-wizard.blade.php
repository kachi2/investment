        ...
        <form method="post" action="{{ route('LaravelInstaller::environmentSaveWizard') }}" class="tabs-wrap">
            <div class="tab" id="tab1content">
                ...
                <div class="form-group {{ $errors->has('is_https') ? ' has-error ' : '' }}">
                    ...
                    <label for="its_https">
                        <input type="radio" name="is_https" id="its_https" value="true" {{ request()->getScheme() == 'https' ? 'checked' : 'disabled'  }} />
                        {{ __('true') }} ({{ __('if SSL available in server') }})
                    </label>
                    <label for="its_not_https">
                        <input type="radio" name="is_https" id="its_not_https" value="false" {{ request()->getScheme() == 'http' ? 'checked' : ''  }} />
                        {{ __('false') }}
                    </label>
                    @if ($errors->has('is_https'))
                        <span class="error-block">
                            <i class="fa fa-fw fa-exclamation-triangle" aria-hidden="true"></i>
                            {{ $errors->first('is_https') }}
                        </span>
                    @endif
                </div>
